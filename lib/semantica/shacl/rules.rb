...
module Semantica
  module Shacl
    # PLAN_0.12.0 — `Semantica::Shacl::Rules` — SHACL Rules
    ...
    module Rules
      ...
      class Engine
        ...
        def discover_rules
          ...
          if unknown.any?
            return ::Semantica::Shacl::Rules.send(:refused,
        ...
        def run_fixpoint(rules)
          ...
          if fixpoint
            ...
            ::Semantica::Shacl::Rules.send(:refused,
        ...
        def focus_meets_condition?(focus_iri, condition_shape_iri)
          transient_report = "urn:semantica:rules:transient-report:#{SecureRandom.uuid}"
          transient_shapes = "urn:semantica:rules:transient-shapes:#{SecureRandom.uuid}"
          ...
          report = ::Semantica::Shacl.validate(
            data_graph:   @data_graph,
          ...
          ::Semantica::Sparql.execute("CLEAR GRAPH <#{transient_shapes}>")
          ::Semantica::Sparql.execute("CLEAR GRAPH <#{transient_report}>")
        ...
        def read_select(graph, sparql)
          r = ::Semantica::Sparql.select(sparql, graph: graph)
          return [] unless r[:ok]
          r[:results]
        end
