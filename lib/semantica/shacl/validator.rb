...
module Semantica
  module Shacl
    ...
    #
    # Internal — operators consume via `Semantica::Shacl.validate(...)`.
    class Validator
      ...
      def clear_report_graph!
        ::Semantica::Sparql.execute("CLEAR GRAPH <#{@report_graph}>")
      end
      ...
      def write_report
        report_node = "urn:semantica:validation-report:#{SecureRandom.uuid}"
        ...
        @violations.each do |v|
          result_node = "urn:semantica:validation-result:#{SecureRandom.uuid}"
        ...
        ::Semantica::Sparql.bulk_insert(rows, raw: true) unless rows.empty?
      end
