...
# Semantica::EtherealGraph with an Active Storage-enabled
# ActiveRecord::Base get auto-registered attachments; operators
# without Active Storage define the `semantica_graph_blob`
...
module Semantica
  ...
  # `has_one_attached :semantica_graph_blob`; the operator's DSL
  ...
  module EtherealGraph
    ...
    included do
      ...
      # loaded at all, leave `semantica_graph_blob` for the operator
      ...
      if respond_to?(:has_one_attached)
        has_one_attached :semantica_graph_blob, dependent: :purge_later
      end
    ...
    def hydrate_ethereal_graph!
      ...
      attachment = semantica_graph_blob
      ...
      rows.each_slice(HYDRATION_BATCH_SIZE) do |chunk|
        result = ::Semantica::Sparql.bulk_insert(chunk, raw: true)
    ...
    def checkpoint_ethereal_graph!
      ...
      begin
        construct = ::Semantica::Sparql.construct(CHECKPOINT_CONSTRUCT_QUERY, graph: iri)
    ...
    def retract_ethereal_graph!
      ...
      iri = instance_exec(&decl.iri_lambda)
      clear = ::Semantica::Sparql.execute("CLEAR GRAPH <#{iri}>")
    ...
    class << self
      ...
      def parse_ntriples(text, graph_iri)
        ...
        text.each_line do |raw_line|
          ...
          next if line.empty?
          terms = ::Semantica::Sparql.send(:split_ntriple, line)
