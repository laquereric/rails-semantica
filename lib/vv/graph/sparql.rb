...
module Vv::Graph
  ...
  #
  #   Vv::Graph::Sparql.select(query, graph: nil)
  #     success → { ok: true,  results: [{ "var" => "value", ... }, ...] }
  #     failure → { ok: false, reason: <symbol>, because: <verbatim engine message> }
  #
  #   Vv::Graph::Sparql.ask(query, graph: nil)
  #     success → { ok: true,  value: true|false }
  #
  #   Vv::Graph::Sparql.construct(query, graph: nil)
  #     success → { ok: true,  ntriples: "<s> <p> <o> .\n..." }
  #
  #   Vv::Graph::Sparql.execute(update_query, graph: nil)
  ...
  #
  # The facade calls Vv::Graph::Loader.ensure_extension_loaded! at
  ...
  module Sparql
    ...
    # `Vv::Graph.rdf_star_writes_enabled?` introspects, so once
    ...
    # `scope:` (Vv::Graph::Scope value object). The Scope's
    ...
    # scalars.
    #
    #   Vv::Graph::Sparql.store_size
    #     → { ok: true, count: <integer> }  # rdf_count_all — every graph
    #
    #   Vv::Graph::Sparql.store_size(graph: nil)
    #     → { ok: true, count: <integer> }  # rdf_count — default graph only
    #
    #   Vv::Graph::Sparql.store_size(graph: "urn:mm:graph:bhphoto")
    ...
    class << self
      ...
      def notify_change_set_from_ntriples(method, body, graph)
        ...
        ::Vv::Graph::ChangeSet.send(method, rows)
      rescue ::Vv::Graph::ChangeSet::ScopeMismatch
        nil
      end
      ...
      def normalize_bulk_rows(rows)
        ...
        rows.map.with_index do |row, idx|
          ...
          s_bare = unwrap_iri(::Vv::Graph::Storable::TermSerializer.iri(s))
          p_bare = unwrap_iri(::Vv::Graph::Storable::TermSerializer.predicate(p))
          o_term = ::Vv::Graph::Storable::TermSerializer.object(o)
