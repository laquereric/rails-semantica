...
module Vv::Graph
  # PLAN_0.9.0 — `Vv::Graph::Reasoner` — forward-chaining OWL 2 RL.
  #
  ...
  module Reasoner
    ...
    PROV_DERIVED_BY   = "urn:vv-graph:derivedBy"
    PROV_DERIVED_AT   = "urn:vv-graph:derivedAt"   # reserved for Phase E.1
    PROV_DERIVED_FROM = "urn:vv-graph:derivedFrom" # reserved for Phase E.1

    def self.rule_iri(rule_id)
      "urn:vv-graph:reasoner:rule:#{rule_id}"
    end
    ...
    # asserted: / inferred: pair or a `scope:` (Vv::Graph::Scope).
      ...
      def resolve_rules(rules)
        ...
        when :owl_2_rl
          ...
          refused(
            ...
            "rules: #{rules.inspect} did not resolve (known: :owl_2_rl, or a Vv::Graph::Reasoner::RuleSet)",
          )
        end
      end
