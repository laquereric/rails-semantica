  ...
  describe "EtherealGraph hydrate of N-Triples-star content", :requires_extension do
    let(:test_class) do
      Class.new do
        ...
        attr_reader :id, :semantica_graph_blob

        def initialize(id, blob_attachment)
          @id = id
          @semantica_graph_blob = blob_attachment
        end
    ...
    it "round-trips a blob containing quoted-triple subjects through hydrate" do
      ...
      size = Semantica::Sparql.store_size(graph: "urn:test:scope:99")
      expect(size[:count]).to eq(3)

      # Specifically: the quoted-triple-subject statement survives
      meta_query = Semantica::Sparql.select(
      ...
      Semantica::EtherealGraph.evict!("urn:test:scope:99")
    end
  end

  describe "Semantica.checkpoint_can_round_trip?(:ntriples_star)" do
    it "now returns true (Phase B flipped the capability)" do
      expect(Semantica.checkpoint_can_round_trip?(content_kind: :ntriples_star)).to be(true)
    end
  end
end
