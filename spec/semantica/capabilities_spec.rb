...
RSpec.describe Semantica, "capability predicates (Phase A)" do
  describe ".rdf_star_writes_enabled?" do
    ...
    it "tracks whether Sparql.quoted_triple is defined (PLAN_0.8.0 Phase B)" do
      ...
      defined_in_facade = Semantica::Sparql.respond_to?(:quoted_triple)
  ...
  describe ".facade_version" do
    ...
    it "matches Semantica::VERSION at v0.13.0 release" do
      expect(described_class.facade_version).to eq(Semantica::VERSION)
    end
  ...
  describe ".checkpoint_can_round_trip?" do
    ...
    it "pins the known content_kinds list" do
      expect(Semantica::CHECKPOINT_CONTENT_KINDS).to eq(%i[plain_ntriples ntriples_star])
    end
  end
end
