...
RSpec.describe Semantica::Reasoner, "OWL 2 RL Phase B", :requires_extension do
  ...
  def inferred_triples(predicate: nil)
    ...
    r = Semantica::Sparql.select("SELECT * WHERE { #{pattern} }", graph: inferred)
  ...
  describe "Rules::OwlRl library" do
    it "contains the Phase B core subset of 15 rules" do
      ids = Semantica::Reasoner::Rules::OwlRl.map(&:id)
    ...
    it "names the W3C OWL 2 RL/RDF rules deferred to Phase B.1" do
      expect(Semantica::Reasoner::Rules::PHASE_B_PENDING).to include(
