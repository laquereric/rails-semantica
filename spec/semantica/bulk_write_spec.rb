...
RSpec.describe "Semantica::Sparql bulk write" do
  describe "module surface" do
    it "exposes bulk_insert and bulk_delete" do
      expect(Semantica::Sparql).to respond_to(:bulk_insert, :bulk_delete)
    end
  end
  ...
  describe "empty input contract" do
    context "without a live extension" do
      ...
      it "bulk_insert([]) returns an :ar_connection_error refusal" do
        ...
        # engine.
        result = Semantica::Sparql.bulk_insert([])
  ...
  describe "round-trip against a live extension", :requires_extension do
    ...
    it "bulk_insert of N Hash-form rows inserts N quads" do
      ...
      result = Semantica::Sparql.bulk_insert(rows)
    ...
    it "bulk_insert of Array-form rows yields identical engine state" do
      ...
      Semantica::Sparql.bulk_insert(hash_rows)
      hash_state = Semantica::Sparql.select(
        "SELECT ?s ?o WHERE { ?s <schema:name> ?o }",
      )[:results].sort_by { |r| r["s"] }

      Semantica::Sparql.execute("CLEAR ALL")
      Semantica::Sparql.bulk_insert(array_rows)
      array_state = Semantica::Sparql.select(
        "SELECT ?s ?o WHERE { ?s <schema:name> ?o }",
      )[:results].sort_by { |r| r["s"] }
    ...
    it "graph-tagged rows route to the named graph" do
      ...
      Semantica::Sparql.bulk_insert(rows)

      default_state = Semantica::Sparql.select(
        "SELECT ?s WHERE { ?s <schema:name> ?o }",
      )[:results].map { |r| r["s"] }
      named_state = Semantica::Sparql.select(
        "SELECT ?s WHERE { ?s <schema:name> ?o }",
        graph: "urn:g:bhphoto",
      )[:results].map { |r| r["s"] }
    ...
    it "one malformed row aborts the whole batch; envelope carries row index" do
      ...
      before_count = Semantica::Sparql.select(
      ...
      after_count = Semantica::Sparql.select(
        "SELECT (COUNT(*) AS ?n) WHERE { ?s ?p ?o }",
      )[:results].first["n"]
  ...
  describe "validation" do
    ...
    it "non-Array input refuses without raising" do
      result = Semantica::Sparql.bulk_insert("not an array")
