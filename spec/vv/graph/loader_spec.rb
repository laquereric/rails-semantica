...
# PLAN 0.29.1 Phase B — Vv::Graph::Loader contract.
#
...
RSpec.describe Vv::Graph::Loader do
  let(:tmp_dir) { Dir.mktmpdir }
  after { FileUtils.rm_rf(tmp_dir) }

  describe Vv::Graph::Loader::ExtensionMissing do
    let(:error) do
    ...
    it "names the VV_GRAPH_SQLITE_SPARQL_PATH override" do
      expect(error.message).to include("VV_GRAPH_SQLITE_SPARQL_PATH")
    end
  end

  describe ".searched_paths" do
    it "lists at least the three platform candidates (macOS / Linux / Windows)" do
      expect(Vv::Graph::Loader.searched_paths.size).to be >= 3
    end

    it "returns absolute paths" do
      Vv::Graph::Loader.searched_paths.each do |p|
  ...
  describe ".extension_path" do
    around do |example|
      ...
      ENV.delete("VV_GRAPH_SQLITE_SPARQL_PATH")
      example.run
      ENV["VV_GRAPH_SQLITE_SPARQL_PATH"] = saved_env
    end

    it "prefers VV_GRAPH_SQLITE_SPARQL_PATH when set + on disk" do
      tmp = File.join(tmp_dir, "libsqlite_sparql.dylib")
      File.write(tmp, "stub")
      ENV["VV_GRAPH_SQLITE_SPARQL_PATH"] = tmp
      expect(Vv::Graph::Loader.extension_path).to eq(tmp)
    end

    it "ignores VV_GRAPH_SQLITE_SPARQL_PATH that doesn't exist on disk" do
      ENV["VV_GRAPH_SQLITE_SPARQL_PATH"] = "/tmp/nonexistent-#{SecureRandom.hex(8)}.dylib"
      # Falls through to searched_paths; nil unless a real extension
      # happens to be present at the default path.
      expect(Vv::Graph::Loader.extension_path).to satisfy { |v| v.nil? || v.is_a?(String) }
    end
  ...
  describe ".ensure_extension_loaded!" do
    ...
    it "raises ExtensionMissing when no extension is on disk" do
      expect { Vv::Graph::Loader.ensure_extension_loaded! }
        .to raise_error(Vv::Graph::Loader::ExtensionMissing) do |error|
          expect(error.message).to include("cargo build --release")
        end
    end
  end
  ...
  describe "AR-less environment" do
    ...
    it "returns :no_active_record when ActiveRecord::Base isn't defined" do
      ...
      expect(Vv::Graph::Loader.ensure_extension_loaded!).to eq(:no_active_record)
    end
  end
  ...
  describe ".engine_version" do
    it "returns :unknown without ActiveRecord loaded" do
      hide_const("ActiveRecord::Base") if defined?(::ActiveRecord::Base)
      expect(Vv::Graph::Loader.engine_version).to eq(Vv::Graph::Loader::ENGINE_VERSION_UNKNOWN)
    end
