...
# PLAN_0.1.0 Phase D — Vv::Graph::Storable contract.
#
# Three layers:
#
...
#      updates replace.
RSpec.describe Vv::Graph::Storable do
  describe Vv::Graph::Storable::TermSerializer do
    describe ".iri" do
  ...
  describe Vv::Graph::Storable::Recorder do
  ...
  describe "lifecycle integration", :requires_extension do
    ...
    it "honors `if:` guards (skips predicates whose guard is falsy)" do
      ...
      yes = Vv::Graph::Sparql.ask("ASK { <urn:mm:widget:W2> <schema:name>  ?o }")
      no  = Vv::Graph::Sparql.ask("ASK { <urn:mm:widget:W2> <schema:price> ?o }")
    ...
    it "retracts triples on destroy" do
      ...
      Vv::Graph::Sparql.ask("ASK { <urn:mm:widget:W4> ?p ?o }").tap do |before|
        expect(before).to eq(ok: true, value: true)
      end

      w.destroy!

      after = Vv::Graph::Sparql.ask("ASK { <urn:mm:widget:W4> ?p ?o }")
    ...
    it "treats nil values as retraction (predicate cleared)" do
      ...
      result = Vv::Graph::Sparql.ask("ASK { <urn:mm:widget:W5> <schema:name> ?o }")
  ...
  describe "PLAN_0.2.0 Phase A — on_subject lifecycle integration", :requires_extension do
    ...
    it "emits primary-subject + on_subject triples on create" do
      ...
      primary = Vv::Graph::Sparql.ask(
        "ASK { <urn:mm:gadget:G1> <schema:name> ?o }",
      )
      derived_type = Vv::Graph::Sparql.select(
        "SELECT ?t WHERE { <urn:mm:folder:category:printer> <rdf:type> ?t }",
      )
      derived_name = Vv::Graph::Sparql.select(
    ...
    it "retracts both primary + on_subject triples on destroy" do
      ...
      after_primary = Vv::Graph::Sparql.ask("ASK { <urn:mm:gadget:G2> ?p ?o }")
      after_folder  = Vv::Graph::Sparql.ask(
        "ASK { <urn:mm:folder:category:scanner> ?p ?o }",
      )
  ...
  describe "PLAN_0.2.0 Phase B — each blocks lifecycle integration", :requires_extension do
    ...
    it "emits one triple per collection item with per-item-interpolated predicates" do
      ...
      weight = Vv::Graph::Sparql.select(
        "SELECT ?o WHERE { <urn:mm:thingy:T1> <mm:weight> ?o }",
      )
      color = Vv::Graph::Sparql.select(
  ...
  describe "PLAN_0.5.0 — `graph \"…\"` declaration", :requires_extension do
    ...
    it "create! emits triples into the named graph, NOT the default graph" do
      ...
      in_graph = Vv::Graph::Sparql.ask(
        "ASK { <urn:mm:gw:G1> <schema:name> ?o }",
        graph: "urn:mm:graph:bhphoto",
      )
      expect(in_graph).to eq(ok: true, value: true)

      in_default = Vv::Graph::Sparql.ask("ASK { <urn:mm:gw:G1> <schema:name> ?o }")
    ...
    it "update! retracts old + inserts new — only in the declared graph" do
      ...
      original = Vv::Graph::Sparql.ask(
        %(ASK { <urn:mm:gw:G2> <schema:name> "Original" }),
        graph: "urn:mm:graph:bhphoto",
      )
      updated = Vv::Graph::Sparql.ask(
      ...
      %w[sparql_update bulk per_call].each do |mode|
        context "in :#{mode} mode" do
          ...
          it "create + update + destroy all confine to the declared graph" do
            ...
            in_graph_a = Vv::Graph::Sparql.ask(
              %(ASK { <urn:mm:gw:GE-#{mode}> <schema:name> "First" }),
              graph: "urn:mm:graph:bhphoto",
            )
            in_default_a = Vv::Graph::Sparql.ask(
              %(ASK { <urn:mm:gw:GE-#{mode}> <schema:name> "First" }),
            )
            ...
            in_graph_b = Vv::Graph::Sparql.ask(
              %(ASK { <urn:mm:gw:GE-#{mode}> <schema:name> "Second" }),
              graph: "urn:mm:graph:bhphoto",
            )
            stale_b = Vv::Graph::Sparql.ask(
              %(ASK { <urn:mm:gw:GE-#{mode}> <schema:name> "First" }),
              graph: "urn:mm:graph:bhphoto",
            )
