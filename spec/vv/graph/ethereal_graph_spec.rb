...
# PLAN_0.7.0 Phase A + B — Vv::Graph::EtherealGraph concern.
#
...
# only requires `vv_graph_blob` to respond to `attached?`
...
RSpec.describe Vv::Graph::EtherealGraph do
  ...
  describe Vv::Graph::EtherealGraph::Recorder do
  ...
  describe "round-trip against a live extension", :requires_extension do
    ...
    it "a record without an attached blob returns :no_blob and skips engine calls" do
      ...
      execute_calls = 0
      bulk_calls = 0
      allow(Vv::Graph::Sparql).to receive(:execute).and_wrap_original do |orig, *a, **kw|
        execute_calls += 1
        orig.call(*a, **kw)
      end
    ...
    it "an empty / whitespace-only blob marks hydrated with :empty_blob" do
      ...
      expect(Vv::Graph::EtherealGraph.hydrated?(host.ethereal_graph_iri)).to be(true)
    end
    ...
    it "malformed blob refuses via bulk_insert envelope; IRI stays unhydrated" do
      ...
      expect(Vv::Graph::EtherealGraph.hydrated?(host.ethereal_graph_iri)).to be(false)
    end
    ...
    it "checkpoint → evict → re-hydrate round-trips engine state" do
      ...
      expect(host.vv_graph_blob.attached?).to be(true)

      Vv::Graph::Sparql.execute("CLEAR GRAPH <#{iri}>")
      Vv::Graph::EtherealGraph.evict!(iri)
    ...
    it "empty-graph checkpoint produces a 0-byte blob; re-hydrate is a no-op" do
      ...
      expect(host.vv_graph_blob.attached?).to be(true)
      expect(host.vv_graph_blob.byte_size).to eq(0)
    ...
    it "propagates a refusal envelope from Sparql.construct without touching the blob" do
      ...
      allow(Vv::Graph::Sparql).to receive(:construct).and_return(refusal)

      result = host.checkpoint_ethereal_graph!
      expect(result).to eq(refusal)
      expect(host.vv_graph_blob.download).to eq(original_text)
    end
  end

  # PLAN_0.7.0 Phase C — retract via before_destroy.
  # Phase D — Vv::Graph::Storable composition.
  #
  ...
  describe "AR-backed lifecycle", :requires_extension do
    ...
    describe "Phase C — destroy retracts the graph" do
      ...
      it "before_destroy fires retract_ethereal_graph! and clears the named graph" do
        ...
        before = Vv::Graph::Sparql.ask("ASK { ?s ?p ?o }", graph: iri)
        expect(before).to eq(ok: true, value: true)

        host.destroy!

        after = Vv::Graph::Sparql.ask("ASK { ?s ?p ?o }", graph: iri)
        expect(after).to eq(ok: true, value: false)
        expect(Vv::Graph::EtherealGraph.hydrated?(iri)).to be(false)
      end
      ...
      it "retraction is graph-scoped — sibling graphs and default graph survive" do
        ...
        own = Vv::Graph::Sparql.ask("ASK { ?s ?p ?o }", graph: iri)
        sibling = Vv::Graph::Sparql.ask("ASK { ?s ?p ?o }", graph: "urn:mm:graph:sibling")
        default = Vv::Graph::Sparql.ask("ASK { ?s ?p ?o }")
    ...
    describe "Phase D — Storable + EtherealGraph composition" do
      ...
      it "create → checkpoint → evict → hydrate round-trips Storable-emitted state" do
        ...
        expect(host.vv_graph_blob.attached?).to be(true)
        expect(host.vv_graph_blob.byte_size).to be > 0
      ...
      it "update! re-emits and re-checkpoints (blob stays in sync)" do
        ...
        first_size = host.vv_graph_blob.byte_size

        host.update!(name: "Composed-Two-Updated-With-Longer-Name")
        expect(host.vv_graph_blob.byte_size).to be > first_size
