...
RSpec.describe "Scope: kwarg integration (PLAN_0.13.0 Phase D)", :requires_extension do
  ...
  describe "Sparql.{select,ask,construct,execute}" do
    ...
    let(:scope) { Vv::Graph::Scope.new(data: data_graph) }

    it "Sparql.select: per-kwarg vs. scope: produce identical output" do
      via_kwarg = Vv::Graph::Sparql.select("SELECT ?o WHERE { <urn:s> <urn:p> ?o }", graph: data_graph)
      via_scope = Vv::Graph::Sparql.select("SELECT ?o WHERE { <urn:s> <urn:p> ?o }", scope: scope)
      expect(via_kwarg).to eq(via_scope)
      expect(via_kwarg[:results]).to contain_exactly("o" => "<urn:o>")
    end

    it "Sparql.ask: per-kwarg vs. scope:" do
      via_kwarg = Vv::Graph::Sparql.ask("ASK { <urn:s> <urn:p> <urn:o> }", graph: data_graph)
      via_scope = Vv::Graph::Sparql.ask("ASK { <urn:s> <urn:p> <urn:o> }", scope: scope)
      expect(via_kwarg).to eq(via_scope)
      expect(via_kwarg[:value]).to be(true)
    end

    it "Sparql.construct: per-kwarg vs. scope:" do
      via_kwarg = Vv::Graph::Sparql.construct(
        "CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }", graph: data_graph,
      )
      via_scope = Vv::Graph::Sparql.construct(
    ...
    it "Sparql.execute: per-kwarg vs. scope:" do
      via_kwarg = Vv::Graph::Sparql.execute('INSERT DATA { <urn:x> <urn:p> <urn:y> . }', graph: data_graph)
      Vv::Graph::Sparql.execute('DELETE DATA { <urn:x> <urn:p> <urn:y> . }', graph: data_graph)
      via_scope = Vv::Graph::Sparql.execute('INSERT DATA { <urn:x> <urn:p> <urn:y> . }', scope: scope)
  ...
  describe "Reasoner.materialise!" do
    let(:scope) { Vv::Graph::Scope.new(data: data_graph, inferred: inferred) }
    ...
    it "per-kwarg vs. scope: produce identical inferred-graph state" do
      r1 = Vv::Graph::Reasoner.materialise!(asserted: data_graph, inferred: inferred)
      # Reset inferred to compare fresh
      Vv::Graph::Sparql.execute("CLEAR GRAPH <#{inferred}>")
      r2 = Vv::Graph::Reasoner.materialise!(scope: scope)
    ...
    it "refuses :scope_kwarg_conflict with overlapping asserted:" do
      r = Vv::Graph::Reasoner.materialise!(
        scope:    scope,
    ...
    it "refuses :scope_role_missing when scope omits inferred" do
      partial = Vv::Graph::Scope.new(data: data_graph)
      r = Vv::Graph::Reasoner.materialise!(scope: partial)
  ...
  describe "Shacl.validate" do
    ...
    it "refuses :scope_role_missing when scope omits shapes:" do
      partial = Vv::Graph::Scope.new(data: data_graph)
      r = Vv::Graph::Shacl.validate(scope: partial)
      expect(r).to include(ok: false, reason: :scope_role_missing)
    end
  end
  ...
  describe "Shacl::Rules.materialise!" do
    ...
    it "per-kwarg vs. scope: equivalence on an empty-rule case" do
      r1 = Vv::Graph::Shacl::Rules.materialise!(
        data_graph: data_graph, shapes_graph: shapes_graph, inferred: inferred,
      )
      r2 = Vv::Graph::Shacl::Rules.materialise!(scope: scope)
      expect(r1).to eq(r2)
    end

    it "refuses :scope_role_missing when scope omits inferred:" do
      partial = Vv::Graph::Scope.new(data: data_graph, shapes: shapes_graph)
      r = Vv::Graph::Shacl::Rules.materialise!(scope: partial)
      expect(r).to include(ok: false, reason: :scope_role_missing)
    end
  end
  ...
  describe ":scope_read_write_overlap" do
    ...
    it "Reasoner refuses with :scope_read_write_overlap" do
      r = Vv::Graph::Reasoner.materialise!(scope: overlap_scope)
